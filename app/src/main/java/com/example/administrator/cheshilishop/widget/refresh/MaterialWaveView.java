...
public class MaterialWaveView extends View implements MaterialHeadListener {
    ...
    public void onPull(MaterialRefreshLayout br, float fraction) {
        setHeadHeight((int) (WdigetUtil.dip2px(getContext(), DefaulHeadHeight) * WdigetUtil.limitValue(1, fraction)));
        setWaveHeight((int) (WdigetUtil.dip2px(getContext(), DefaulWaveHeight) * Math.max(0, fraction - 1)));
        invalidate();
    }
    ...
    public void onRefreshing(MaterialRefreshLayout br) {
        setHeadHeight((WdigetUtil.dip2px(getContext(), DefaulHeadHeight)));
        ValueAnimator animator = ValueAnimator.ofInt(getWaveHeight(), 0);
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                Log.i("anim", "value--->" + animation.getAnimatedValue());
